<template>
    <div class="flex items-center justify-between mb-8">
        <h2 class="text-xl">Roles List</h2>
        <button-create :href="$route('authorization.roles.create')">create</button-create>
    </div>
            ...
            <table class="min-w-full text-theme-color">
                <thead class="bg-primary">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-100 uppercase tracking-wider">Permissions</th>
                ...
                <tbody class="bg-theme-bg divide-y divide-gray-200 border-b border-gray-200">
                    <tr v-for="(role, index) in roles" :key="index">
                        ...
                        <td class="px-6 py-3 space-x-1">
                            <span v-for="permission in role.permissions" :key="permission.id" class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                               {{ permission.name }}
                            </span>
                        </td>
                        <td class="px-6 py-3 whitespace-nowrap border-r border-gray-200">
                            <div class="flex items-center justify-end space-x-2">
                                <icon-edit :href="$route('authorization.roles.edit', { role: role.id })"></icon-edit>
                                <icon-delete @click="openModal({id:role.id, name:role.name})"></icon-delete>
                            </div>
                        </td>
                    </tr>
                </tbody>
...
export default {
    ...
    setup() {
        ...
        function openModal(role) {
            ...
            modalTitle.value = 'Are you sure? You want to delete role: <span class="px-2 py-1 text-base text-dark font-semibold bg-red-200 rounded-lg">' + role.name + '</span> ?'
        }

        function deleteData(id) {
            Inertia.delete(route('authorization.roles.destroy', { role: id }), {
